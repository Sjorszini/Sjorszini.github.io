(function () {
  ...
  var COLLISION_CONTACT_MARGIN = 0.04;
  var SUPPORT_CONTACT_MARGIN = 0.14;
  var RESTITUTION_SPEED_THRESHOLD = 0.45;
  var REST_SEPARATION_SPEED = 0.12;
  var REST_SPEED = 0.22;
  var FLOOR_REST_SPEED = 0.32;
  ...
  function exactGravity() {
    gravityMode = "exact + contact support";
    for (var i = 0; i < NumberOfBalls; i += 1) {
      var a = ball[i];
  ...
  function clusteredGravity() {
    gravityMode = "clustered + contact support";
  ...
  function calculateForces() {
    resetForces();
    if (!MutualGravity) {
      gravityMode = VerticalGravity ? "vertical + contact support" : "off";
      return;
    }
  ...
  function pairGeometry(a, b, margin) {
    var dx = b.x - a.x;
    var dy = b.y - a.y;
    var minDistance = a.r + b.r;
    var distance2 = dx * dx + dy * dy;
    var contactDistance = minDistance + margin;
    ...
    return { nx: nx, ny: ny, distance: distance, penetration: minDistance - distance };
  }

  function forEachNearbyPair(margin, callback, countCheck) {
    if (!Collisions || NumberOfBalls < 2) return;
    var cellSize = Math.max(12, 2 * maximumRadius() + margin);
      ...
      for (var ox = -1; ox <= 1; ox += 1) {
        for (var oy = -1; oy <= 1; oy += 1) {
          ...
          for (var n = 0; n < bucket.length; n += 1) {
            var j = bucket[n];
            if (j <= i) continue;
            if (countCheck) collisionChecks += 1;
            callback(a, ball[j]);
          }
        }
      }
  ...
  function supportPair(a, b) {
    var g = pairGeometry(a, b, SUPPORT_CONTACT_MARGIN);
    if (!g) return;
    ...
    var axB = b.Fx * invB;
    var ayB = b.Fy * invB;
    var relativeNormalAcceleration = (axB - axA) * g.nx + (ayB - ayA) * g.ny;

    if (relativeNormalAcceleration < 0) {
      var force = -relativeNormalAcceleration / invSum;
      var fx = force * g.nx;
      var fy = force * g.ny;
      a.Fx -= fx;
      a.Fy -= fy;
  ...
  function supportWalls() {
    if (!Boundaries) return;
    for (var i = 0; i < NumberOfBalls; i += 1) {
      var b = ball[i];
      if (b.x - b.r <= SUPPORT_CONTACT_MARGIN && b.Fx < 0) b.Fx = 0;
      if (width - (b.x + b.r) <= SUPPORT_CONTACT_MARGIN && b.Fx > 0) b.Fx = 0;
      if (b.y - b.r <= SUPPORT_CONTACT_MARGIN && b.Fy < 0) b.Fy = 0;
      if (height - (b.y + b.r) <= SUPPORT_CONTACT_MARGIN && b.Fy > 0) b.Fy = 0;
    }
  }

  function solveStaticContactForces() {
    if (!Collisions) return;
    var iterations = NumberOfBalls <= 180 ? 10 : (NumberOfBalls <= 1100 ? 7 : 4);
    for (var pass = 0; pass < iterations; pass += 1) {
      forEachNearbyPair(SUPPORT_CONTACT_MARGIN, supportPair, false);
      supportWalls();
    }
  }
  ...
  function solvePairVelocity(a, b, firstPass) {
    var g = pairGeometry(a, b, COLLISION_CONTACT_MARGIN);
    if (!g) return;
    ...
    if (vn < 0) {
      ...
      b.vx += ix * invB;
      b.vy += iy * invB;
    } else if (EnergyDissipation > 0 && vn < REST_SEPARATION_SPEED) {
      var settleImpulse = -vn / invSum;
      var sx = settleImpulse * g.nx;
      var sy = settleImpulse * g.ny;
      a.vx -= sx * invA;
      a.vy -= sy * invA;
      b.vx += sx * invB;
      b.vy += sy * invB;
  ...
  function projectPair(a, b) {
    ...
    var correction = Math.max(g.penetration - 0.0005, 0) * 0.96 / invSum;
  ...
  function solveVelocityContacts(firstPass) {
    forEachNearbyPair(COLLISION_CONTACT_MARGIN, function (a, b) {
      solvePairVelocity(a, b, firstPass);
    }, true);
  }

  function projectContacts() {
    forEachNearbyPair(COLLISION_CONTACT_MARGIN, projectPair, true);
  }
  ...
  function solveWallVelocities(firstPass) {
    ...
    for (var i = 0; i < NumberOfBalls; i += 1) {
      ...
      if (b.x - b.r <= COLLISION_CONTACT_MARGIN) {
        b._leftContact = true;
        b._contactCount += 1;
        if (b.vx < 0) b.vx = (-b.vx >= RESTITUTION_SPEED_THRESHOLD && firstPass) ? -b.vx * e : 0;
      }
      ...
      if (b.y - b.r <= COLLISION_CONTACT_MARGIN) {
        ...
        b._contactCount += 1;
        if (b.vy < 0) b.vy = (-b.vy >= RESTITUTION_SPEED_THRESHOLD && firstPass) ? -b.vy * e : 0;
      }
  ...
  function projectWalls() {
    ...
    for (var i = 0; i < NumberOfBalls; i += 1) {
      var b = ball[i];
      if (b.x < b.r) { b.x = b.r; b._leftContact = true; b._contactCount += 1; }
      else if (b.x > width - b.r) { b.x = width - b.r; b._rightContact = true; b._contactCount += 1; }
      if (b.y < b.r) { b.y = b.r; b._ceilingContact = true; b._contactCount += 1; }
      else if (b.y > height - b.r) { b.y = height - b.r; b._floorContact = true; b._contactCount += 1; }
    }
  }
  ...
  function stabilizeRestingContacts() {
    ...
    for (var i = 0; i < NumberOfBalls; i += 1) {
      ...
      if ((Boundaries || VerticalGravity) && b._contactCount > 0) {
  ...
  function physicsStep(stepDt) {
    calculateForces();
    solveStaticContactForces();
    integrate(stepDt);
    resetContactState();

    var velocityIterations = NumberOfBalls <= 180 ? 5 : 3;
    for (var v = 0; v < velocityIterations; v += 1) {
      solveVelocityContacts(v === 0);
      solveWallVelocities(v === 0);
    }

    var positionIterations = NumberOfBalls <= 180 ? 8 : (NumberOfBalls <= 1100 ? 7 : 4);
  ...
  function motionStats() {
    ...
    return { internalKinetic: internalKinetic, comSpeed: Math.sqrt(cvx * cvx + cvy * cvy) };
  }
  ...
  function updateStats(now) {
    ...
    if (output) {
      ...
      output.textContent = NumberOfBalls.toLocaleString() + " balls · " + fps + " fps · " +
        collisionChecks.toLocaleString() + " contact checks/frame · gravity: " + gravityMode +
