...
const files=['finsler-enhancement-01-validation.js','finsler-enhancement-02-dependencies.js','finsler-enhancement-03-result-tools.js','finsler-enhancement-04-cancel.js','finsler-enhancement-05-share-state.js','finsler-enhancement-06-catalogue-tags.js'];
files.forEach(name=>{if(!loader.includes(name))fail('loader missing '+name);});
const m={validation:read(files[0]),dependencies:read(files[1]),results:read(files[2]),cancel:read(files[3]),share:read(files[4]),catalogue:read(files[5])};
...
if(!/expand\(x,defs\)/.test(m.share))fail('catalogue definitions are not made self-contained in shared metric state');
if(!/item\.validation/.test(m.catalogue)||!/item\.description/.test(m.catalogue))fail('catalogue search does not include validation/description metadata');
if(!/stopImmediatePropagation/.test(m.catalogue)||!/forceCoreRenderAll/.test(m.catalogue))fail('enhanced catalogue search does not safely supersede core search filtering');
...
const sample={v:1,dimension:4,coords:['t','r','theta','phi'],metric:[['-(1-r_s/r)']],outputs:{ricci:true}};const encoded=Buffer.from(JSON.stringify(sample),'utf8').toString('base64url');const decoded=JSON.parse(Buffer.from(encoded,'base64url').toString('utf8'));if(JSON.stringify(decoded)!==JSON.stringify(sample))fail('share-state roundtrip failed');

if(!process.exitCode)console.log('PASS: enhancement checks through catalogue metadata/filtering pass');
