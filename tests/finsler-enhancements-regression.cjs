...
const files=['finsler-enhancement-01-validation.js','finsler-enhancement-02-dependencies.js','finsler-enhancement-03-result-tools.js','finsler-enhancement-04-cancel.js','finsler-enhancement-05-share-state.js','finsler-enhancement-06-catalogue-tags.js','finsler-enhancement-07-mobile-flow.js','finsler-enhancement-08-display-prefs.js','finsler-enhancement-09-accessibility.js','finsler-enhancement-10-display-copy-sync.js'];
files.forEach(name=>{if(!loader.includes(name))fail('loader missing '+name);});
const m={validation:read(files[0]),dependencies:read(files[1]),results:read(files[2]),cancel:read(files[3]),share:read(files[4]),catalogue:read(files[5]),mobile:read(files[6]),display:read(files[7]),a11y:read(files[8]),copySync:read(files[9])};
...
expect(m.a11y,['Site navigation','Einstein quotation','Search metric catalogue','Filter metric catalogue by group','Filter metric catalogue by mathematical tag','aria-labelledby','aria-describedby','aria-errormessage','aria-live','Calculation results','finsler-sr-only','focus-visible','FINSLER_ACCESSIBILITY_AUDIT','FINSLER_ACCESSIBILITY_API'],'accessibility');
expect(m.copySync,['FINSLER_DISPLAY_PREFS','transformSource','finslerDisplayBase','finslerCopySource','MutationObserver','requestAnimationFrame','FINSLER_DISPLAY_COPY_SYNC'],'display/copy sync');
...
if(!/main\.dataset\.finslerCopySource=window\.FINSLER_DISPLAY_PREFS\.transformSource\(main\.dataset\.finslerDisplayBase\)/.test(m.copySync))fail('copied LaTeX is not synchronized to the final display notation');
...
if(!process.exitCode)console.log('PASS: all requested enhancement checks, accessibility semantics, long-expression copy safeguards, and display/copy synchronization pass');
