...
const files=['finsler-enhancement-01-validation.js','finsler-enhancement-02-dependencies.js','finsler-enhancement-03-result-tools.js','finsler-enhancement-04-cancel.js','finsler-enhancement-05-share-state.js','finsler-enhancement-06-catalogue-tags.js','finsler-enhancement-07-mobile-flow.js'];
files.forEach(name=>{if(!loader.includes(name))fail('loader missing '+name);});
const m={validation:read(files[0]),dependencies:read(files[1]),results:read(files[2]),cancel:read(files[3]),share:read(files[4]),catalogue:read(files[5]),mobile:read(files[6])};
...
if(!/results\.children\.length>0/.test(m.mobile)||!/b\.disabled=!has/.test(m.mobile))fail('Results navigation is not disabled until results exist');
...
if(!process.exitCode)console.log('PASS: enhancement checks through mobile workflow navigation pass');
